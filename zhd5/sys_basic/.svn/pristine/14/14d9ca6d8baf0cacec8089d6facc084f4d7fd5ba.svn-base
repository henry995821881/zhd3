package com.xyscm.sys.basic.vo;

import java.io.Serializable;
import java.math.BigDecimal;
import java.util.Date;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class BasicEmployeeVo implements Serializable{
	private static final long serialVersionUID = 1L;

	private BigDecimal employeeId;

    private String memberCode;

    private String employeeCode;

    private String employeeName;

    private String employeeMnemcode;

    private String orgCode;

    private String deptCode;

    private String worktypeName;

    private String employeeJob;

    private String employeeSex;

    private String employeeIdcard;

    private String employeeNative;

    private String employeeSpecialty;

    private String employeeParty;

    private String employeeTechnical;

    private String employeeNation;

    private String employeeMarriage;

    private String employeeDegree;

    private String employeeBirthday;

    private String employeeJoindate;

    private String employeeAddr;

    private String employeePhone;

    private String employeeMobile;

    private String employeeEmail;

    private String employeeClass;

    private BigDecimal employeeState;

    private String employeeRemark;

    private BigDecimal basicShare;

    public BigDecimal getEmployeeId() {
        return employeeId;
    }

    public void setEmployeeId(BigDecimal employeeId) {
        this.employeeId = employeeId;
    }

    public String getMemberCode() {
        return memberCode;
    }

    public void setMemberCode(String memberCode) {
        this.memberCode = memberCode == null ? null : memberCode.trim();
    }

    public String getEmployeeCode() {
        return employeeCode;
    }

    public void setEmployeeCode(String employeeCode) {
        this.employeeCode = employeeCode == null ? null : employeeCode.trim();
    }

    public String getEmployeeName() {
        return employeeName;
    }

    public void setEmployeeName(String employeeName) {
        this.employeeName = employeeName == null ? null : employeeName.trim();
    }

    public String getEmployeeMnemcode() {
        return employeeMnemcode;
    }

    public void setEmployeeMnemcode(String employeeMnemcode) {
        this.employeeMnemcode = employeeMnemcode == null ? null : employeeMnemcode.trim();
    }

    public String getOrgCode() {
        return orgCode;
    }

    public void setOrgCode(String orgCode) {
        this.orgCode = orgCode == null ? null : orgCode.trim();
    }

    public String getDeptCode() {
        return deptCode;
    }

    public void setDeptCode(String deptCode) {
        this.deptCode = deptCode == null ? null : deptCode.trim();
    }

    public String getWorktypeName() {
        return worktypeName;
    }

    public void setWorktypeName(String worktypeName) {
        this.worktypeName = worktypeName == null ? null : worktypeName.trim();
    }

    public String getEmployeeJob() {
        return employeeJob;
    }

    public void setEmployeeJob(String employeeJob) {
        this.employeeJob = employeeJob == null ? null : employeeJob.trim();
    }

    public String getEmployeeSex() {
        return employeeSex;
    }

    public void setEmployeeSex(String employeeSex) {
        this.employeeSex = employeeSex == null ? null : employeeSex.trim();
    }

    public String getEmployeeIdcard() {
        return employeeIdcard;
    }

    public void setEmployeeIdcard(String employeeIdcard) {
        this.employeeIdcard = employeeIdcard == null ? null : employeeIdcard.trim();
    }

    public String getEmployeeNative() {
        return employeeNative;
    }

    public void setEmployeeNative(String employeeNative) {
        this.employeeNative = employeeNative == null ? null : employeeNative.trim();
    }

    public String getEmployeeSpecialty() {
        return employeeSpecialty;
    }

    public void setEmployeeSpecialty(String employeeSpecialty) {
        this.employeeSpecialty = employeeSpecialty == null ? null : employeeSpecialty.trim();
    }

    public String getEmployeeParty() {
        return employeeParty;
    }

    public void setEmployeeParty(String employeeParty) {
        this.employeeParty = employeeParty == null ? null : employeeParty.trim();
    }

    public String getEmployeeTechnical() {
        return employeeTechnical;
    }

    public void setEmployeeTechnical(String employeeTechnical) {
        this.employeeTechnical = employeeTechnical == null ? null : employeeTechnical.trim();
    }

    public String getEmployeeNation() {
        return employeeNation;
    }

    public void setEmployeeNation(String employeeNation) {
        this.employeeNation = employeeNation == null ? null : employeeNation.trim();
    }

    public String getEmployeeMarriage() {
        return employeeMarriage;
    }

    public void setEmployeeMarriage(String employeeMarriage) {
        this.employeeMarriage = employeeMarriage == null ? null : employeeMarriage.trim();
    }

    public String getEmployeeDegree() {
        return employeeDegree;
    }

    public void setEmployeeDegree(String employeeDegree) {
        this.employeeDegree = employeeDegree == null ? null : employeeDegree.trim();
    }


    public String getEmployeeBirthday() {
		return employeeBirthday;
	}

	public void setEmployeeBirthday(String employeeBirthday) {
		this.employeeBirthday = employeeBirthday;
	}

	public String getEmployeeJoindate() {
		return employeeJoindate;
	}

	public void setEmployeeJoindate(String employeeJoindate) {
		this.employeeJoindate = employeeJoindate;
	}

	public String getEmployeeAddr() {
        return employeeAddr;
    }

    public void setEmployeeAddr(String employeeAddr) {
        this.employeeAddr = employeeAddr == null ? null : employeeAddr.trim();
    }

    public String getEmployeePhone() {
        return employeePhone;
    }

    public void setEmployeePhone(String employeePhone) {
        this.employeePhone = employeePhone == null ? null : employeePhone.trim();
    }

    public String getEmployeeMobile() {
        return employeeMobile;
    }

    public void setEmployeeMobile(String employeeMobile) {
        this.employeeMobile = employeeMobile == null ? null : employeeMobile.trim();
    }

    public String getEmployeeEmail() {
        return employeeEmail;
    }

    public void setEmployeeEmail(String employeeEmail) {
        this.employeeEmail = employeeEmail == null ? null : employeeEmail.trim();
    }

    public String getEmployeeClass() {
        return employeeClass;
    }

    public void setEmployeeClass(String employeeClass) {
        this.employeeClass = employeeClass == null ? null : employeeClass.trim();
    }

    public BigDecimal getEmployeeState() {
        return employeeState;
    }

    public void setEmployeeState(BigDecimal employeeState) {
        this.employeeState = employeeState;
    }

    public String getEmployeeRemark() {
        return employeeRemark;
    }

    public void setEmployeeRemark(String employeeRemark) {
        this.employeeRemark = employeeRemark == null ? null : employeeRemark.trim();
    }

    public BigDecimal getBasicShare() {
        return basicShare;
    }

    public void setBasicShare(BigDecimal basicShare) {
        this.basicShare = basicShare;
    }
    public String toString() {
		return ReflectionToStringBuilder.toString(this);
	}
}