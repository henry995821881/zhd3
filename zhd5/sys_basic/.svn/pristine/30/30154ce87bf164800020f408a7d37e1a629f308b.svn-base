//必须定位id #pageHtml
var mask = $('.mask'),Prompt_success=$('#Prompt_success'),Prompt_confirm=$('#Prompt_confirm');
...
//提示框
var PromptBox= function(PromptMsg,PromptTitle,PromptType,PromptF){
	//确认
	...
	switch(PromptType) {
	case '1':
		var PromptHtml='<div class="box"><div class="Prompt_text">'+PromptMsg+'</div><div class="Prompt_button"><button class="Prompt_confirm">确认</button><button class="Prompt_cancel">取消</button></div></div>';
		$('#Prompt_confirm').html(PromptHtml);
		PromptConfirm();
		Prompt_confirm();
		Prompt_cancel();
		break;
	case '2':
		var PromptHtml='<div class="box"><div class="Prompt_text">'+PromptMsg+'</div><div class="Prompt_button"><button class="Prompt_confirm">确认</button></div></div>';
		$('#Prompt_confirm').html(PromptHtml);
		PromptConfirm();
		Prompt_confirm();
		break;
...
//删除提示框关闭
	var PromptConfirmHide=function(){
//		mask.hide();
		Prompt_confirm.hide();
	}
	//删除成功提示框
	...
	//删除成功提示框关闭
	var PromptSuccessHide=function(){
		mask.hide();
		Prompt_success.hide();
	}
//关闭窗口
...
//		jPages();
	...
	table_detail.off('click');
	table_detail.on('click', function(e) {
	...
	/*add_table_icon.off('click');
	add_table_icon.on('click', function() {
		add_table_show(add_table,mask);
		table_detail_empty();
	});*/
	add_table_close.off('click');
